...
def cadastrar_turma_disciplinas_base_comum(planilha, sheet):
    try:
        wb = openpyxl.load_workbook(planilha)
        sheet = wb[sheet]
        materias_bc = []
        ...
        for bc in materias_bc:
            conexao_bd = conexao.iniciar_conexao()
            query = "INSERT INTO base_comum_disciplinas (nome_disciplina, ano_escolar, ano_realizacao, carga_horaria, frequencia) VALUES (%s, %s, %s, %s, %s, %s);"
            parametro = (bc[2], bc[3], bc[4], bc[5], bc[6])
            cursor_bd = conexao_bd.cursor()
            cursor_bd.execute(query, parametro)
            conexao_bd.commit()
            cursor_bd.close()
...
def exibir_aluno(id_aluno, id_curso):
    conexao_bd = conexao.iniciar_conexao()
    query_aluno = "SELECT * FROM aluno WHERE id = " + str(id_aluno) +";"
    query_curso = "SELECT * FROM curso_tecnico WHERE id = " + str(id_aluno) +";"
    cursor_bd = conexao_bd.cursor()

    cursor_bd.execute(query_aluno, query_curso)
...
def editar_aluno(nome_aluno, nome_mae, nome_pai, cpf_aluno, municipio_aluno, estado_aluno, rg_aluno, turma_aluno, nome_curso, eixo_tecnologico):
    ...
    query_aluno = "UPDATE curso_tecnico SET nome_curso = %s, eixo tecnologico = %s;"
    parametros_curso = (nome_curso, eixo_tecnologico)

    cursor_bd = conexao_bd.cursor()

    cursor_bd.execute(query_aluno, parametros_aluno, parametros_curso)
    conexao_bd.commit()
    cursor_bd.close()
    conexao_bd.close()
